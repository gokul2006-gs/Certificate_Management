...
function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      api.get("/admin/login-logs?page=1&pageSize=25"),
    ])
      .then(([statsRes, logsRes]) => {
        if (active) {
          setStats(statsRes.data);
          setLoginLogs(logsRes.data.items || logsRes.data);
        }
      })
      .catch((error) => {
        if (active) {
      ...
      <PageHeader title="Admin Dashboard" eyebrow="Operations Overview">
        <div className="flex gap-2">
          <button 
            ...
            className="inline-flex items-center gap-2 rounded-xl bg-emerald-600 px-4.5 py-2.5 text-xs font-bold text-white hover:bg-emerald-700 transition-all duration-300 hover:scale-[1.02] shadow-md shadow-emerald-600/10 active:scale-[0.98] disabled:opacity-50"
