...
function Students() {
  ...
  const load = async (nextPage = page, search = query) => {
    const r = await api.get("/students", { params: { page: nextPage, pageSize: 12, search } });
    setStudents(r.data.items || r.data);
  ...
  const allSelected = students.length > 0 && students.every((s) => selectedIds.includes(s.student_id));

  const handleCreate = async (e) => {
    e.preventDefault();
    setMessage("");
    try {
      await api.post("/students", { name: form.name.trim(), email: form.email.trim().toLowerCase() });
      setForm({ name: "", email: "" });
      setMessage("Student registered with a hashed default password.");
      load(1, query);
    } catch (err) {
  ...
  const saveEdit = async (id) => {
    setMessage("");
    try {
      await api.put(`/students/${id}`, { name: editForm.name.trim(), email: editForm.email.trim().toLowerCase() });
      setEditingId(""); setMessage("Student updated."); load(page, query);
  ...
  const deleteOne = async (id) => {
    ...
    setMessage("Student deleted."); load(page, query);
  };

  const toggleSelect = (id) => setSelectedIds((p) => p.includes(id) ? p.filter((x) => x !== id) : [...p, id]);
  const toggleAll = () => {
    if (allSelected) { const s = new Set(students.map((x) => x.student_id)); setSelectedIds((p) => p.filter((x) => !s.has(x))); }
    else { const m = new Set(selectedIds); students.forEach((x) => m.add(x.student_id)); setSelectedIds(Array.from(m)); }
  };
  ...
  const deleteBulk = async () => {
    ...
    setSelectedIds([]); setMessage(`${selectedIds.length} students deleted.`); load(1, query);
  };
      ...
      <section className="rounded-2xl border border-slate-200/60 bg-white shadow-sm overflow-hidden animate-fade-in-up">
        <div className="flex flex-col gap-4 border-b border-slate-100 p-5 bg-slate-50/50 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-[10px] text-slate-400">{total} profiles</p>
          </div>
        ...
        <Pagination page={page} totalPages={totalPages} onPageChange={(next) => load(next, query)} />
      </section>
    </Layout>
  );
}
